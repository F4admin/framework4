import {
	createMutationObjectFromSchema,
	createMutationObject,
} from "./orm-factory"
import {
	createQueryResolver,
	createMutationResolver,
} from "./default-query-resolver"
...
function createWithSchemaAndHeaders(endpoint, schema, headers) {
	return createWithResolverAndSchema(
		createQueryResolver(endpoint, headers),
		schema
	)
}

function createWithSchema(endpoint, schema, auth) {
	return createWithSchemaAndHeaders(
		endpoint,
		schema,
		auth ? { Authorization: `BMToken ${auth}` } : {}
	)
}
