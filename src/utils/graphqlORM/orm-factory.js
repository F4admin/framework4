...
function createMutationFunction(mutationResolver, mutationObject) {
	return async function mutationFunction(args, returnFields) {
		...
		const nonNullableTypes = mutationObject.args.filter(
			(a) => a.type.kind === "NON_NULL"
		)

		// Check if we have missing or extra args
		const missingArgs = nonNullableTypes.filter(
			(a) => args[a.name] === undefined
		)
		const extraArgs = Object.keys(args).filter(
			(a) => mutationObject.args.filter((x) => a === x.name).length === 0
		)

		if (missingArgs.length > 0) {
			throw new Error(
				`The following arguments are missing: ${missingArgs
					.map((a) => a.name)
					.join(", ")}`
			)
		}
		...
		if (extraArgs.length > 0) {
			throw new Error(
				`The following extra arguments were passed: ${extraArgs.join(
					", "
				)}`
			)
		}
		...
		// Build the query
		const varDecl = variables
			.map((k) => `$${k.name}: ${k.type}`)
			.join(", \n")
		const varAssignment = variables
			.map((k) => `${k.name}: $${k.name}`)
			.join(", \n")

		const mutation = `mutation dynamic_mutation ${
			varDecl
				? `(
            ${varDecl} 
          )`
...
function createQueryFunction(queryResolver, mutationObject) {
	return async function mutationFunction(args, info) {
		...
		const query = `query dynamic_query ${
			varDecl
				? `(
            ${varDecl} 
          )`
...
export function createMutationObjectFromSchema(
	queryResolver,
	mutationResolver,
	schema
) {
...
export async function createMutationObject(queryResolver, mutationResolver) {
	...
	return createMutationObjectFromSchema(
		queryResolver,
		mutationResolver,
		schema
	)
