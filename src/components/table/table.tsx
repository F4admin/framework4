...
import TableTunnel, { State as TableState, Column } from '../../utils/tableTunnel'
...
export class Table {
    ...
    async _fetchData () {
        ...
        if (!this.isEmpty(graphqlApi) && columns.length !== 0) {
            ...
            const data = isConnection ? this._unfoldDataConnection(await graphqlApi[queryConnection](queryWhere, info)) : await graphqlApi[query](queryWhere, info)
            this.loading = false
            this.count = count
            this.data = data
            console.log(data)
